...
import IntCoord qualified as I
...
data Opaque2L
  = forall a b.
    (IsCoordList '[a, b], AllDiffSame Int '[a, b]) =>
    Opaque2L (Proxy '[a, b])
...
opaqueSweepL k (Opaque2L (_ :: Proxy '[a, b])) =
  total
    [ (if isJust (offsetCoord c (k :| k :| EmptyCoord)) then 1 else 0)
        + (if onBoundary c then 1 else 0)
        + coordDistance c zeroCoord
    | c <- allCoord @'[a, b]
    ]
...
opaqueSweepI k (Opaque2I (_ :: Proxy '[a, b])) =
  total
    [ (if isJust (I.offsetI c (k :. k :. DEnd)) then 1 else 0)
        + (if I.onBoundaryI c then 1 else 0)
        + I.coordDistanceI c I.zeroCoordI
    | c <- I.allCoordI @'[a, b]
    ]
