...
3. **`Reflective` and `Reflect101` disagree about what a checked step means.**
   One stops at the wall, the other turns around. `Reflect101` is the only
   axis in the library where a step the bounds check *accepts* also reports a
   frame flip, at the mirror cell where `mirrorAt`'s documented tie-break
   fires. Filed as its own bug; the design doc has the table.
