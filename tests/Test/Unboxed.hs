    ...
    [ testGroup
        ...
        [ testCase "ugridFromList lays out row-major, as gridFromList does" $
          ...
          testCase "tabulateGrid agrees with tabulateGrid" $
            (tabulateGrid coordPosition :: UGrid Square Int)
              `sameAs` (tabulateGrid coordPosition :: Grid Square Int),
      ...
      testGroup
        ...
        [ testCase "mapGrid" $
          ...
          testCase "imapGrid" $
            imapGrid (\c x -> coordPosition c + x) unboxed
              `sameAs` imapGrid (\c x -> coordPosition c + x) boxed,
          testCase "zipWithGrid" $
            zipWithGrid (+) unboxed unboxed
              `sameAs` zipWithGrid (+) boxed boxed,
          testCase "scanl1Grid" $
        ...
        [ testCase "transposeGrid" $
          ...
          testCase "shrinkGrid takes the same window" $
            ( shrinkGrid (zeroCoord :: Coord '[Ordinal 2, Ordinal 2]) unboxed ::
                UGrid '[Ordinal 3, Ordinal 3] Int
            )
              `sameAs` shrinkGrid (zeroCoord :: Coord '[Ordinal 2, Ordinal 2]) boxed,
