...
-- is the defect the note on 'Data.Grid.Sized.Coord.Class.IsCoordList' describes,
...
-- 360,000 on a bare @'Data.Grid.Sized.Coord.Clamped.Clamped' 300@ axis as a control:
--
...
-- list is the 94 KB.
--
-- == Why not a method of 'Data.Grid.Sized.Coord.Class.IsCoordList'
--
-- The per-axis step needs @'AffineSpace' x@, which
-- 'Data.Grid.Sized.Coord.Class.IsCoordLifted' does not supply, and adding
...
-- being indexable: 'Data.Grid.Sized.Ordinal.Ordinal' is an
-- 'Data.Grid.Sized.Coord.Class.IsCoord' with no 'AffineSpace' instance at all.
--
...
-- 'Data.Grid.Sized.Coord.Class.offsetIsCoord'. The whole offset succeeds only if
...
-- 'Data.Grid.Sized.Coord.Class.npOffset', an 'IsCoordList' method, for the reason
...
-- two 'Data.Grid.Sized.Coord.Class.offsetIsCoord' calls with no axis-list dictionary
...
-- axes answer for --- 'Data.Grid.Sized.Coord.Class.offsetIsCoord' applies a whole
...
--
-- A 'Data.Grid.Sized.Coord.Periodic.Periodic' axis never refuses a step, so on an
...
-- all-'Data.Grid.Sized.Coord.Periodic.Periodic' coord and every ray with a zero
...
stepsWithin ::
    ...
    -> [(Int, Coord cs)]
-- The fold this used to carry as a @where@ helper is now
-- 'Data.Grid.Sized.Coord.Class.npStepsWithin', an 'IsCoordList' method, for the
...
-- 'Data.Grid.Sized.Coord.Periodic.Periodic' one takes the shorter way round.
...
-- the interior.
--
-- The lifted form of 'Data.Grid.Sized.Coord.Class.axisBoundaryIsCoord', so each axis
...
-- everywhere on an all-'Data.Grid.Sized.Coord.Periodic.Periodic' coord, and 'False'
