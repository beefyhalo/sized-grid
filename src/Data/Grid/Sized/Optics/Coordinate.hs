...
import           Data.Grid.Sized.Coord         (AffineCoordList, Coord, MapDiff,
                                                StrengthenCoord, WeakenCoord,
                                                appendCoord, coordFromPosition,
                                                coordFromTuple, coordPosition,
                                                coordSplit, coordToTuple,
                                                singleCoord, strengthenCoord,
                                                transposeCoord, unCoord,
                                                unsafeCoordFromPosition,
                                                weakenCoord,
                                                pattern EmptyCoord)
import           Data.Grid.Sized.Coord.Class   (Boundaryless, IsCoord,
                                                IsCoordLifted, IsCoordList (..),
                                                coordListSize, strengthenIsCoord,
                                                toAxisIndex, unsafeFromAxisIndex,
                                                weakenIsCoord)
import           Data.Grid.Sized.Coord.Delta   (Delta (..), appendDelta,
                                                deltaFromTuple, deltaSplit,
                                                deltaToTuple)
import           Data.Grid.Sized.Internal.Type (requiring)
import           Data.Grid.Sized.Ordinal       (Ordinal, numToOrdinal,
                                                ordinalToNum, strengthenOrdinal,
                                                weakenOrdinal)

import           Control.Lens
import           Data.AdditiveGroup            (AdditiveGroup (..))
import           Data.AffineSpace              (Diff, (.+^))
import           Generics.SOP                  (All, I (..), IsProductType,
                                                NP (..))
