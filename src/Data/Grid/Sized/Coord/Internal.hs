...
instance (IsCoordList cs, All ToJSON cs) => ToJSON (Coord cs) where
  toJSON c =
    Array $
      V.fromList $
        hcollapse $
          hcmap (Proxy @ToJSON) (\(I x) -> K $ toJSON x) (unCoord c)
...
instance
  forall cs.
  (IsCoordList cs, All AdditiveGroup cs) =>
  AdditiveGroup (Coord cs)
  where
    ...
    case coordFromPosition @cs p of
      ...
      Nothing ->
        error $
          "toEnum: "
            ++ show p
            ++ " is out of range for Coord "
            ++ show (coordListSize @cs)
...
instance
  forall cs.
  (IsCoordList cs, All Semigroup cs, All Monoid cs, All Group cs) =>
  Group (Coord cs)
  where
...
-- each axis.
instance
  (IsCoordList cs, All Semigroup cs, All Monoid cs, All Group cs, All Abelian cs) =>
  Abelian (Coord cs)
...
appendCoord ::
  forall a as.
  (IsCoordLifted a, IsCoordList as) =>
  a ->
  Coord as ->
  Coord (a ': as)
...
instance
  (IsCoordLifted x, AffineSpace x, AffineCoordList xs) =>
  AffineCoordList (x ': xs)
  where
...
data SizeProof (cs :: [Type]) where
  ...
  SizeCons ::
    forall c n cs.
    (GHC.KnownNat n, AllSizedKnown cs) =>
    SizeProof (c n ': cs)
...
instance
  (GHC.KnownNat n, AllSizedKnown as) =>
  AllSizedKnown (c n ': as)
  where
  sizeProof = SizeCons
