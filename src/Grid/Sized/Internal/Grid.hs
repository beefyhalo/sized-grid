...
-- |
-- Module      :  Grid.Sized.Internal.Grid
...
-- with "Grid.Sized.Grid.Unsafe" without also being shared with the world: the
...
-- to break it. "Grid.Sized.Grid.Grid" re-exports the safe half of what is here.
--
...
import           Grid.Sized.Internal.Type (requiring, windowFits)
...
-- | The escape hatch, re-exported from "Grid.Sized.Grid.Unsafe".
--
...
-- suffices at the call site. 'Grid.Sized.Coord.AllSizedKnown' has always been a
...
-- comes from @CoordNat@ on the `Grid.Sized.Coord.Class.IsCoordLifted` instance,
...
-- 'Grid.Sized.Ordinal.Ordinal' no longer carries that dictionary in every value.
instance ( KnownNat x
         , KnownNat z
         , AllSizedKnown as
         , IsCoord c
